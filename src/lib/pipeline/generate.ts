...
import type { McqPayload, NumericPayload, ClozePayload, OrderPayload, LocatePayload, ChainPayload, MapPayload } from "@/lib/content/card-payloads";
...
/**
 * Deterministic whole-corpus card generator (no AI call — templates +
 * distractor logic only). Scans every verified+unsuppressed fact each run
 * and skips any fact_id:type combo that already has a card, so it's cheap
 * and idempotent to re-run after any content-pipeline step adds new facts
 * (the JIT pipeline calls this after every extraction instead of trying to
 * book-scope the generation itself).
 */
export async function generateCards(admin: DB): Promise<{ cardsCreated: number }> {
  const [{ data: entities }, { data: facts }, { data: books }, { data: timelineEvents }, { data: genealogyEdges }, { data: geoPlaces }, { data: verses }] =
    await Promise.all([
      ...
      admin.from("facts").select("id, entity_id, fact_key, fact_value, numeric_val, unit, verse_ref, book_id, chapter").eq("verified", true).eq("suppressed", false),
      ...
      admin.from("verses").select("id, book_id, chapter, verse, text"),
    ]);
  ...
  // Persistent dedup so re-running this after every JIT extraction doesn't
  // re-insert the same card twice. recall/reverse/numeric/mcq are keyed by
  // fact_id (unique per card), but cloze/locate/order/chain/map have
  // fact_id=null — each needs its own natural key built from what's
  // actually unique about it, checked against every existing card of that
  // type, not just a same-run Set (that was the bug: the original script's
  // clozeSeen/etc. sets only caught duplicates *within* one run — a second
  // run started fresh and re-inserted everything all over again).
  const { data: existingCards } = await admin.from("cards").select("fact_id, type, entity_id, book_id, chapter, verse_ref, answer, tags");
  const existingKey = new Set<string>();
  for (const c of existingCards ?? []) {
    if (c.type === "recall" || c.type === "reverse" || c.type === "numeric" || c.type === "mcq") {
      existingKey.add(`${c.fact_id}:${c.type}`);
    } else if (c.type === "cloze") {
      existingKey.add(`cloze:${c.verse_ref}`); // one cloze per verse, regardless of which entity got blanked
    } else if (c.type === "locate") {
      existingKey.add(`locate:${c.book_id}:${c.chapter}`);
    } else if (c.type === "order") {
      existingKey.add(`order:${c.answer}`);
    } else if (c.type === "chain") {
      existingKey.add(`chain:${c.entity_id}:${(c.tags ?? []).join(",")}`);
    } else if (c.type === "map") {
      existingKey.add(`map:${c.entity_id}`);
    }
  }
  ...
  function difficultyFor(type: CardType, entity: EntityRow | undefined, numericVal: number | null) {
    return calibrateDifficulty({ entityImportance: entity?.importance, entityRefCount: entity?.ref_count, numericVal, type });
  }
  ...
  for (const v of verseRows) {
    ...
    if (existingKey.has(`cloze:${verseRef}`)) continue; // already has a cloze card, regardless of which entity
